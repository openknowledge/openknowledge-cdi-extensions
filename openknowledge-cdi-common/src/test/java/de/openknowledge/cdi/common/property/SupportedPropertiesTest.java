...
public class SupportedPropertiesTest {
  ...
  public void testCurrentProperties() throws Exception {
    ...
    assertTrue(properties.getDistinctPropertyKeys().contains("testString"));
    assertTrue(properties.getDistinctPropertyKeys().contains("testInt"));

    for (ApplicationProperty property : properties.getDistinctProperties()) {
      if (property.getPropertyKey().equals("testString")) {
        return;
      }
    }

    fail("Property key 'testString' not found.");
  }
}
